...
import { isAuthorOnlyCategory } from "@/lib/author-only";
...
export default function CategoryContent({
  ...
  const [authorCategoryIds, setAuthorCategoryIds] = useState<string[]>([]);
  ...
  useEffect(() => {
    fetch("/api/me", { credentials: "include" })
      ...
      .then((d) => {
        ...
        setAuthorCategoryIds(Array.isArray(d.user?.authorCategoryIds) ? d.user.authorCategoryIds : []);
      })
  ...
  const IMAGE_ONLY_CATEGORIES = ["delicious-food"];
  const isImageOnlyCategory = IMAGE_ONLY_CATEGORIES.includes(categoryId);
  const canPost = !isAuthorOnlyCategory(categoryId) || isAdmin || (authorCategoryIds?.includes(categoryId) ?? false);
        ...
        {canPost && (
          <button
            ...
            style={{
              padding: "0.4rem 0.9rem",
              fontSize: "0.85rem",
              background: showPostForm ? "var(--glass)" : "var(--mag-primary)",
              border: showPostForm ? "1px solid var(--glass-border)" : "1px solid rgba(255,255,255,0.2)",
              borderRadius: "6px",
              color: showPostForm ? "var(--gold-dim)" : "#fff",
              cursor: "pointer",
            }}
          >
            Post
