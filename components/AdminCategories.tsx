...
export default function AdminCategories() {
  ...
  const [newMenuSection, setNewMenuSection] = useState("main");
  ...
  useEffect(() => {
    function loadSections() {
      fetch("/api/menu-sections", { credentials: "include" })
        .then((r) => r.json())
        .then((d) => {
          const secs = d.sections || [];
          setSections(secs);
          setNewMenuSection((prev) => prev || secs[0]?.id || "main");
        })
        .catch(() => {});
    }
    loadSections();
  ...
  function handleSubDrop(e: React.DragEvent, dropSubId: string, parentId: string, children: { id: string; name: string }[]) {
    e.preventDefault();
    setDragOverSubId(null);
    ...
    if (res.ok) {
      setCategories((prev) =>
        prev.map((c) => ({
          ...
          children: c.children?.map((ch) => (ch.id === catId ? { ...ch, defaultTab } : ch)) ?? [],
        }))
      );
      window.dispatchEvent(new Event("categories-updated"));
    } else {
  ...
  async function handleCreate() {
    ...
    if (!newParentId) body.menuSection = newMenuSection || sections[0]?.id || "main";
    const res = await fetch("/api/admin/categories", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      credentials: "include",
              ...
              <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: "0.5rem", flexWrap: "wrap", gap: "0.5rem" }}>
                {editingId === cat.id ? (
                  <div style={{ display: "flex", alignItems: "center", gap: "0.5rem", flex: 1, minWidth: 0 }}>
                    ...
                    />
                    <button type="button" onClick={saveEdit} style={{ padding: "0.25rem 0.5rem", fontSize: "0.75rem", background: "var(--mag-primary, #530b0f)", border: "none", borderRadius: "4px", color: "#fff", cursor: "pointer" }}>
                      Save
                    </button>
                    <button type="button" onClick={cancelEditing} style={{ padding: "0.25rem 0.5rem", fontSize: "0.75rem", background: "var(--glass)", border: "1px solid var(--glass-border)", borderRadius: "4px", color: "var(--gold-dim)", cursor: "pointer" }}>
                      Cancel
                    </button>
                  </div>
                ) : (
                ...
                >
                  ...
                  {sections.length === 0 && (
                    <>
                      ...
                      <option value="culinary">Culinary Traveler</option>
                    </>
                  )}
                </select>
                <select
                  ...
                  {cat.children.map((ch) => (
                    <div
                      ...
                      style={{
                        ...
                        borderTop: dragOverSubId === ch.id ? "2px solid var(--mag-primary, var(--gold))" : "2px solid transparent",
                      }}
                    >
                      ...
                      {editingId === ch.id ? (
                        <div style={{ display: "flex", alignItems: "center", gap: "0.35rem", flex: 1, minWidth: 0 }}>
                          ...
                          />
                          <button type="button" onClick={saveEdit} style={{ padding: "0.2rem 0.4rem", fontSize: "0.7rem", background: "var(--mag-primary, #530b0f)", border: "none", borderRadius: "4px", color: "#fff", cursor: "pointer" }}>
                            Save
                          </button>
