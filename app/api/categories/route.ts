...
import { MAG_FALLBACK_TREE } from "@/lib/categories";
...
export async function GET() {
  try {
    ...
    if (dbCategories.length === 0) {
      const res = NextResponse.json({ tree: MAG_FALLBACK_TREE });
    ...
    const tree = parents
      ...
      .map((p) => ({
        id: p.id,
        name: p.name,
        menuSection: p.menuSection || "main",
    ...
    const fallback = NextResponse.json({ tree: MAG_FALLBACK_TREE });
