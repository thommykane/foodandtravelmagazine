...
import { posts, users, categoryFollows, categories, votes, sessions, moderators } from "@/lib/db/schema";
...
import { isAuthorOnlyCategory, AUTHOR_EMAIL } from "@/lib/author-only";
...
export async function POST(req: NextRequest) {
  try {
    ...
    if (isAuthorOnlyCategory(categoryId)) {
      const isAdminEmail = user.email.toLowerCase() === AUTHOR_EMAIL.toLowerCase();
      const [mod] = await db
        .select()
        .from(moderators)
        .where(and(eq(moderators.userId, user.id), eq(moderators.categoryId, categoryId)))
        .limit(1);
      if (!isAdminEmail && !mod) {
        return NextResponse.json(
          { error: "Only the editor or authors can post in this category." },
          { status: 403 }
        );
      }
    }
