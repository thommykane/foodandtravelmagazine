...
export async function POST(req: NextRequest) {
  ...
  const maxSort = await db
    ...
    .from(categories)
    .where(eq(categories.parentId, parentId ?? null));
  ...
  await db.insert(categories).values({
    ...
    sortOrder,
    ...(parentId ? {} : { menuSection: typeof menuSection === "string" && menuSection.trim() ? menuSection.trim() : "main" }),
...
export async function DELETE(req: NextRequest) {
  ...
  const childCount = await db.select().from(categories).where(eq(categories.parentId, id));
